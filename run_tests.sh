...
function usage {
    echo "Usage: $0 [OPTION]..."
    echo "Run Senlin's test suite(s)"
    echo ""
...
wrapper=""
debug=0
flake8args="senlin bin/senlin-api bin/senlin-engine bin/senlin-manage"

function run_tests {
    echo 'Running tests'
    # Remove any extraneous DB migrations
    find senlin/db/sqlalchemy/migrate_repo/versions/ -name '*.pyc' -delete
    ...
    if [ $debug -eq 1 ]; then
      ...
      if [ "$args" = "" ]; then
        ...
        # provided.
        testrargs="discover ./senlin/tests"
      fi
