...
class DBAPILockTest(base.SenlinTestCase):
    ...
    def test_cluster_lock_acquire_success(self):
        observed = db_api.cluster_lock_acquire(self.cluster.id, UUID1)
        self.assertEqual(UUID1, observed)

    def test_cluster_lock_acquire_fail_double_same(self):
        db_api.cluster_lock_acquire(self.cluster.id, UUID1)
        observed = db_api.cluster_lock_acquire(self.cluster.id, UUID1)
        self.assertEqual(UUID1, observed)

    def test_cluster_lock_acquire_fail_double_different(self):
        db_api.cluster_lock_acquire(self.cluster.id, UUID1)
        observed = db_api.cluster_lock_acquire(self.cluster.id, UUID2)
        self.assertEqual(UUID1, observed)
    ...
    def test_cluster_lock_steal_fail_stolen(self):
        ...
        db_api.cluster_lock_release(self.cluster.id, UUID1)
        db_api.cluster_lock_acquire(self.cluster.id, UUID2)
