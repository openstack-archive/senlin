...
class TestBatchPolicy(base.SenlinTestCase):
    ...
    def test__cal_batch_size(self):
        policy = bp.BatchPolicy('test-batch', self.spec)

        size, number = policy._cal_batch_size(5)
        self.assertEqual(2, size)
        self.assertEqual(3, number)

    def test__cal_batch_size_with_default_max(self):
        self.spec['properties']['max_batch_size'] = -1
    ...
    def test__create_plan(self, mock_count, mock_filter,
                          mock_cal, mock_pick):
        action = mock.Mock()
        ...
        min_in_service = self.spec['properties']['min_in_service']
        excepted_plan = {
            'pause_time': self.spec['properties']['pause_time'],
            'min_in_service': min_in_service,
            'plan': [{'1', '2'}, {'3'}]
        }
        self.assertTrue(res)
        self.assertEqual(plan, excepted_plan)

        mock_count.assert_called_once_with(action.context, cluster.id)
        mock_filter.assert_called_once_with(cluster.nodes)
        mock_cal.assert_called_once_with(count)
        mock_pick.assert_called_once_with(2, 2, [], [node1, node2, node3])

    @mock.patch.object(no.Node, 'count_by_cluster')
    def test__create_plan_less_than_min(self, mock_count):
        action = mock.Mock()
        action.context = self.context
        cluster = mock.Mock(id='cid')
        mock_count.return_value = 1
        self.spec['properties']['min_in_service'] = 2

        policy = bp.BatchPolicy('test-batch', self.spec)
        res, reason = policy._create_plan(cluster, action)
        self.assertFalse(res)
        self.assertEqual('The parameter min_in_service is greater than '
                         'current number of nodes.', reason)
        mock_count.assert_called_once_with(action.context, cluster.id)

    @mock.patch.object(no.Node, 'count_by_cluster')
    def test__create_plan_no_node(self, mock_count):
        action = mock.Mock()
        action.context = self.context
        cluster = mock.Mock(id='cid')
    ...
    def test_pre_op(self, mock_load, mock_plan):
        action = mock.Mock()
        action.context = self.context
        cluster = mock.Mock(id='cid')
